...
for m=1:M
    X_pred(:,m) = feval(p_u,X_in(:,m),u) + R*randn(size(X_in(:,1))); %propigate each particle by updating according to the
...
X_bar = [X_pred;w]; %prediction set contains states and weights
w_out = w;
%update step
try
